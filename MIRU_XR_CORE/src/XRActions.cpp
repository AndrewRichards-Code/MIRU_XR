...
ActionSet::ActionSet(CreateInfo* pCreateInfo)
{
	...
	MIRU_XR_ASSERT(m_CI.actionSetName.size() > sizeof(m_ActionSetCI.actionSetName), "ERROR: OPENXR: ActionSet Name is too long.");
	strcpy_s(m_ActionSetCI.actionSetName, m_CI.actionSetName.c_str());
	MIRU_XR_ASSERT(m_CI.localisedActionSetName.size() > sizeof(m_ActionSetCI.localizedActionSetName), "ERROR: OPENXR: Localised ActionSet Name is too long.");
	strcpy_s(m_ActionSetCI.localizedActionSetName, m_CI.localisedActionSetName.c_str());
	m_ActionSetCI.priority = m_CI.priority;

	MIRU_XR_ASSERT(xrCreateActionSet(m_CI.instance->m_Instance, &m_ActionSetCI, &m_ActionSet), "ERROR: OPENXR: Failed to create ActionSet.");
}

ActionSet::~ActionSet()
{
	MIRU_XR_ASSERT(xrDestroyActionSet(m_ActionSet), "ERROR: OPENXR: Failed to destroy ActionSet.");
}
...
Action::Action(CreateInfo* pCreateInfo)
{
	...
	MIRU_XR_ASSERT(m_CI.actionName.size() > sizeof(m_ActionCI.actionName), "ERROR: OPENXR: Action Name is too long.");
	...
	MIRU_XR_ASSERT(m_CI.localisedActionName.size() > sizeof(m_ActionCI.localizedActionName), "ERROR: OPENXR: Localised Action Name is too long.");
	strcpy_s(m_ActionCI.localizedActionName, m_CI.localisedActionName.c_str());
}

Action::~Action()
{
	MIRU_XR_ASSERT(xrDestroyAction(m_Action), "ERROR: OPENXR: Failed to destroy Action.");
}